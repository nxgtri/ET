...
namespace ETPathfinder.PF
{
    public class TileHandler
    {
        ...
        List<NavmeshCut> allCut = new List<NavmeshCut>();
        ...
        public class TileType
        {
			 ...
			 */
            public void Load(out Int3[] verts, out int[] tris)
            {
                verts = this.verts;
                tris = this.tris;
            }
        ...
        internal void AddCut(NavmeshCut navmeshCut)
        {
            allCut.Add(navmeshCut);
        }

        internal void RemoveCut(NavmeshCut navmeshCut)
        {
            allCut.Remove(navmeshCut);
        }
        ...
        void CutPoly(Int3[] verts, int[] tris, ref Int3[] outVertsArr, ref int[] outTrisArr, out int outVCount, out int outTCount, Int3[] extraShape, Int3 cuttingOffset, Bounds realBounds, CutMode mode = CutMode.CutAll | CutMode.CutDual, int perturbate = 0)
        {
            ...
            List<Int3> outverts = ListPool<Int3>.Claim(verts.Length * 2);
            List<int> outtris = ListPool<int>.Claim(tris.Length);
            var sol = new PolyTree();

            var sol2 = new List<List<IntPoint>>();
            var polyCache = new Stack<Pathfinding.Poly2Tri.Polygon>();
            ...
            {
                navmeshCuts = GetAllCutInRange(realBounds);
            }

            List<int> tmpIntersectingCuts = ListPool<int>.Claim();
            //Don't want to write a completely new IntBounds class, so an IntRect and a range for Y axis will have to suffice.
            List<IntRect> cutBounds = ListPool<IntRect>.Claim();
            List<Int2> cutBoundsY = ListPool<Int2>.Claim();
            List<List<IntPoint>> cutVertices = new List<List<IntPoint>>();
            List<bool> cutIsDual = ListPool<bool>.Claim();
            List<bool> cutsAddedGeom = ListPool<bool>.Claim();

            if (perturbate > 10)
            {
                //Log.Error("Too many perturbations aborting : " + mode);
            ...
            while (true)
            {
                ...
                else
                {
                    ...
                    int ct;
                    ct = ClipPolygon(clipIn, 3, clipOut, 1, 0, 0);
                    if (ct == 0)
                        continue;

                    ct = ClipPolygon(clipOut, ct, clipIn, -1, 2 * clipExtents.x, 0);
                    if (ct == 0)
                        continue;

                    ct = ClipPolygon(clipIn, ct, clipOut, 1, 0, 2);
                    if (ct == 0)
                        continue;

                    ct = ClipPolygon(clipOut, ct, clipIn, -1, 2 * clipExtents.z, 2);
                    if (ct == 0)
                        continue;
                ...
                for (int cmode = 0; cmode < 16; cmode++)
                {
                    ...
                    else if (1 << cmode == (int)CutMode.CutDual)
                    {
                        ...
                        {
                            clipper.AddPolygon(sol2[i], Clipper.Orientation(sol2[i]) ? PolyType.ptClip : PolyType.ptSubject);
                        }
                    ...
                    for (int exp = 0; exp < sol.ChildCount; exp++)
                    {
                        ...
                        else
                        {
                            ...
                            Pathfinding.Poly2Tri.Polygon pl = null;
                            ...
                            while (contour != null)
                            {
                                ...
                                Pathfinding.Poly2Tri.Polygon tmpPoly = null;
                                ...
                                {
                                    tmpPoly = new Pathfinding.Poly2Tri.Polygon(polypoints);
                                }
                            ...
                            {
                                //Log.Warn("PointOnEdgeException, perturbating vertices slightly ( at " + cmode + " in " + mode + ")");
                            ...
                            {
                                Pathfinding.Poly2Tri.DelaunayTriangle t = pl.Triangles[i];
#if ASTARDEBUG
                            ...
                            if (pl.Holes != null)
                                for (int i = 0; i < pl.Holes.Count; i++)
                                {
                                    pl.Holes[i].Points.Clear();
                                    pl.Holes[i].ClearTriangles();

                                    if (pl.Holes[i].Holes != null)
                                        pl.Holes[i].Holes.Clear();
            ...
            if (cached_int_array.Length < outverts.Count)
                cached_int_array = new int[Math.Max(cached_int_array.Length * 2, outverts.Count)];
            ...
            for (int i = 0; i < outVCount; i++)
                outVertsArr[i] = outverts[i];

            //Release back to object pool
            ListPool<Int3>.Release(outverts);
            ListPool<int>.Release(outtris);
            ListPool<int>.Release(tmpIntersectingCuts);
            ListPool<Int2>.Release(cutBoundsY);
            ListPool<bool>.Release(cutIsDual);
            ListPool<bool>.Release(cutsAddedGeom);
            ListPool<IntRect>.Release(cutBounds);
        ...
        public List<NavmeshCut> GetAllCutInRange(Bounds b)
        {
            List<NavmeshCut> cuts = ListPool<NavmeshCut>.Claim();
            for (int i = 0; i < allCut.Count; i++)
            {
                if (Intersects(b, allCut[i].GetBounds()))
                {
                    cuts.Add(allCut[i]);
                }
            }
        ...
        */
        ...
        public List<NavmeshCut> GetAllAddInRange(Bounds b)
        {
            List<NavmeshCut> cuts = ListPool<NavmeshCut>.Claim();
            for (int i = 0; i < allCut.Count; i++)
            {
        ...
        Vector3 Point2D2V3(TriangulationPoint p)
        {
		 ...
		 */
        public NavmeshTile ReloadTile(int x, int z)
        {
            if (x < 0 || z < 0 || x >= graph.tileXCount || z >= graph.tileZCount)
                return null;

            int index = x + z * graph.tileXCount;
            if (activeTileTypes[index] != null)
            {
                return LoadTile(activeTileTypes[index], x, z);
            }
            return null;
        }
        ...
        public void CutShapeWithTile(int x, int z, Int3[] shape, ref Int3[] verts, ref int[] tris, out int vCount, out int tCount)
        {
            ...
            activeTileTypes[index].Load(out tverts, out ttris);
		 ...
		 */
        public NavmeshTile LoadTile(TileType tile, int x, int z)
        {
            if (tile == null)
            ...
            var newTile = graph.ReplaceTile(x, z, outVerts, outTris, false);
